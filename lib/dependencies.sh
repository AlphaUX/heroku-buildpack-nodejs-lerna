...
run_build_if_present() {
  ...
  if [[ "$has_script_name" == "true" ]]; then
    if $YARN || $YARN_2; then
      ...
      if [[ -n $NODE_BUILD_FLAGS ]]; then
        echo "Running with $NODE_BUILD_FLAGS flags"
        monitor "${script_name}-script" npx lerna run "$script_name" --scope "$LERNA_SCOPE" --include-dependencies -- --if-present -- "$NODE_BUILD_FLAGS"
      else
        monitor "${script_name}-script" npx lerna run "$script_name" --scope "$LERNA_SCOPE" --include-dependencies -- --if-present
      fi
    fi
  fi
}
